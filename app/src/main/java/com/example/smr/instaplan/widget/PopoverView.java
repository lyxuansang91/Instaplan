...
package com.example.smr.instaplan.widget;
...
public class PopoverView extends RelativeLayout implements OnTouchListener{
	...
	public static interface PopoverViewDelegate{
		...
		void clickItem(PopoverView view, int idButton);
	}
	...
	private void initPopoverView(View viewToEnclose){
		...
		LinearLayout ll_add = (LinearLayout) viewToEnclose.findViewById(R.id.ll_add);
		LinearLayout ll_crop = (LinearLayout) viewToEnclose.findViewById(R.id.ll_crop);
		LinearLayout ll_share = (LinearLayout) viewToEnclose.findViewById(R.id.ll_share);
		LinearLayout ll_delete = (LinearLayout) viewToEnclose.findViewById(R.id.ll_delete);
		ll_add.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				delegate.clickItem(PopoverView.this,0);
			}
		});

		ll_crop.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				delegate.clickItem(PopoverView.this,1);
			}
		});
		...
		ll_delete.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				delegate.clickItem(PopoverView.this,3);
			}
		});
		...
		ll_share.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				delegate.clickItem(PopoverView.this,2);
			}
		});
	...
	private void addArrow(Rect originRect, Integer arrowDirection){
		...
		if (arrowDirection == PopoverView.PopoverArrowDirectionUp){
			...
			yPos = originRect.bottom - popoverLayoutRect.top - 15;
		}
			...
			arrowHeight = arrowDrawable.getIntrinsicHeight();
			xPos = originRect.centerX() - (arrowWidth/2) - popoverLayoutRect.left;
			yPos = originRect.top - arrowHeight - popoverLayoutRect.top + 15;
		}
			...
			arrowWidth = arrowDrawable.getIntrinsicWidth();
			arrowHeight = arrowDrawable.getIntrinsicHeight();
			xPos = originRect.right - popoverLayoutRect.left - 15;
			...
			arrowWidth = arrowDrawable.getIntrinsicWidth();
			arrowHeight = arrowDrawable.getIntrinsicHeight();
			xPos = originRect.left - arrowWidth - popoverLayoutRect.left + 15;
			yPos = originRect.centerY() - (arrowHeight/2) - popoverLayoutRect.top;
		}
		//Set drawable
